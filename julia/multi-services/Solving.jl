...
function Solving_global_prob(D_n,capacity)
    ...
    @NLobjective(prob, Min, sum(As[s]/Theta[s] * (sum( Tx_Power*V_s[s]/(w[n]*up_capacity[n]) for n =1:NumbDevs) +
                            ...
                            kappa * (T_com[s] +T_avg[s] + K_l[s]*T_cmp[s])) for s=1:Numb_Services))
    ...
    for s =1:Numb_Services
        ...
        Obj += K_g[s] * (rs_E_com[s] +K_l[s]*rs_E_cmp[s] + kappa * (rs_T_com[s] + T_avg[s] +K_l[s]*rs_T_cmp[s]))
    end
    println("eta: ", rs_eta)
    println("w: ",rs_w[1:5])
    println("f: ",rs_f[:,1:5])
...
function compute_service_cost(T_cmp, E_cmp, T_com, E_com)
    ...
    for s =1:Numb_Services
        C_s[s] = E_com[s] +K_l[s]*E_cmp[s] + kappa * (T_com[s]+T_avg[s] +K_l[s])*T_cmp[s]
    end
    return C_s
end
...
function Solving_isub_prob2(K_g, capacity, s_idx, v, z)
    ...
    for s =1:numb_serv
        for n=1:NumbDevs
            @NLconstraint(prob, V_s[s_idx]/(w[s,n]*up_capacity[n])+ tau_extra[s,n] + tau_dl[s] <= T_com[s])
        end
    end
