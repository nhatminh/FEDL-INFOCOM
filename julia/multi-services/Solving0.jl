...
function Solving_global_prob(D_n,capacity)
    ...
    for s =1:Numb_Services
        @constraint(prob, Theta[s] == A2s[s]*eta[s]^2 + B2s[s]*eta[s])
        for n=1:NumbDevs
    ...
    @NLobjective(prob, Min, sum(A1s[s]/Theta[s] * (sum( Tx_Power*V_s[s]/(w[n]*up_capacity[n]) for n =1:NumbDevs) +
...
function Solving_sub_prob3( C_s)
    ...
    @NLobjective(prob, Min, sum(A1s[s]*C_s[s]/Theta[s] for s=1:Numb_Services) )
    ...
    for s =1:Numb_Services
        ...
        Obj += A1s[s]*C_s[s]/(A2s[s]*rs_eta[s]^2 + B2s[s]*rs_eta[s])
    end
...
function Solving_isub_prob3(s_idx, C_s)
    ...
    @constraint(prob, Theta[1] == A2s[s_idx]*eta[1]^2 + B2s[s_idx]*eta[1])

    # @NLobjective(prob, Min, sum(A1s[s_idx]*C_s/(A2s[s_idx]*eta[i]^2 + B2s[s_idx]*eta[i]) for i=1:numb_serv) )
    @NLobjective(prob, Min, sum(A1s[s_idx]*C_s/Theta[i] for i=1:numb_serv) )
    ...
    Obj = 0
    for i =1:numb_serv
        Obj += A1s[s_idx]*C_s/(A2s[s_idx]*rs_eta[i]^2 + B2s[s_idx]*rs_eta[i])
    end
...
function Solving_isub3( s_idx, C_s)
    ...
    rs_eta = -B2s[s_idx]/(2*A2s[s_idx])

    Obj = A1s[s_idx]*C_s/(A2s[s_idx]*rs_eta^2 + B2s[s_idx]*rs_eta)
    Obj_E = 0
    Obj_T = 0
    # println("rs_eta: ", rs_eta)
    ...
    # println("Test sub3:", abs(log(rs_eta) + 1/rs_eta - 1/eta))
